...
6. 摘要（给人看，不要粘贴招标正文）：
   ...
   - 专用设备：名称 / 规格 / 数量；投标文件组成：文件名 / 格式 / 是否必须 / 备注；
   - 废标/否决：写成「风险点：关键数字与后果（等级，条款号）」，按开标前/资格/符合性/评标定标分项填写，不要整段须知。
...
8. 原文（给后续 AI 预审）：只摘该字段对应的招标条款原文（含条款号），不要把整章投标人须知粘进来。
   评分类字段原文可与摘要相同，以免漏项。
...
8. 摘要删套话，保留数字、条款号；原文只摘对应条款，不要整章粘贴。
"""
...
def _extract_batch(
    ...
    if addendum:
        user_content = (
            ...
            "每个字段填 {摘要, 原文}。未在答疑中出现的字段摘要和原文都填空。"
            "与招标文件冲突时只保留答疑口径。\n\n"
            f"{catalog}\n\n"
    ...
    if "evalMethod" in dim_keys or "envelope" in dim_keys:
        user_content = (
            "本批次抽取评标/详细评审规则。"
            + (
                "技术评分表进 eval-tech：摘要按「评分项目 + 分值」逐条列出，"
